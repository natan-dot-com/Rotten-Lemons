...
CREATE TABLE SEGUE (
    ...
    CONSTRAINT FK2_SEGUE FOREIGN KEY (NOME_USUARIO_SEGUIDO) REFERENCES 
        USUARIO (NOME_USUARIO) ON DELETE CASCADE,
    CONSTRAINT CK_SEGUE CHECK(NOME_USUARIO_SEGUIDO <> NOME_USUARIO)
);
...
CREATE TRIGGER PERMISSAO_REMOVER_COMENTARIO
    BEFORE INSERT ON COMENTARIO_REMOVIDO
    FOR EACH ROW EXECUTE PROCEDURE TRIGGER_PERMISSAO_REMOVE_COMENTARIO();
...
CREATE TABLE TAG_REMOVIDA (
    ...
    CONSTRAINT PK_TAGREMOVIDA PRIMARY KEY (TAG, MUSICA, NOME_USUARIO),
...
CREATE TRIGGER PERMISSAO_REMOVER_TAG
    BEFORE INSERT ON TAG_REMOVIDA
    FOR EACH ROW EXECUTE PROCEDURE TRIGGER_PERMISSAO_REMOVE_TAG();
